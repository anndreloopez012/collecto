          ...
          <form id="formDataTelefono" method="POST">
            ...
            ?>
                <tr style="cursor:pointer;">
                  <td><a style="text-aline:center;" onclick="fntCall('<?php print $intContador; ?>')"><i class="fad fa-2x fa-phone-square"></i></a></td>
                  <td>
                    <?PHP
...
<?PHP
  ...
  else if ($strTipoValidacion == "llamada_saliente") {
    $intAgente = isset($_POST['agente'])?intval($_POST['agente']):0;
    $intCampaing = isset($_POST['idCampaign'])?intval($_POST['idCampaign']):0;
    $strNumero = isset($_POST['numero'])?$_POST['numero']:'';
    $strUsuario = isset($_SESSION['USUARIO'])?$_SESSION['USUARIO']:'';
    $strClave = isset($_SESSION['CLAVE'])?$_SESSION['CLAVE']:'';
    
    $curl = curl_init();
    
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://callcenter2.gcollecto.com.gt/api/v1/login',
        ...
        CURLOPT_POSTFIELDS => array('username' => $strUsuario,'password' => $strClave),
    ));
    ...
    if( isset($arrResult['token']) ) {
        $strToken = $arrResult['token'];
        $curl = curl_init();
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://callcenter2.gcollecto.com.gt/api/v1/makeCall/',
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => '',
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 0,
          ...
          CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
          CURLOPT_CUSTOMREQUEST => 'POST',
          CURLOPT_POSTFIELDS => array('idCampaign' => $intCampaing,'idAgent' => $intAgente,'phone' => $strNumero),
