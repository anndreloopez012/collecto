...
?>
          ...
          <form id="formDataTelefono" method="POST">
            ...
            ?>
                <tr style="cursor:pointer;">
                  ...
                  <td style="cursor:pointer; background: #a9cce3; " onclick="fntSelectFrontAzteca('<?php print $intContador; ?>')"><?php echo  $rTMP["value"]['NUMERO']; ?></td>
                ...
                <input type="hidden" name="hid_tell_<?php print $intContador; ?>" id="hid_tell_<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['NUMERO']; ?>">
            <?PHP
                $intContador++;
              }
  ...
  <?PHP
    die();
  } else if ($strTipoValidacion == "encrypt_azteca") {
    header('Content-Type: application/json');
    $gt = isset($_POST["gt"]) ? $_POST["gt"]  : 0;

    $pais = isset($_POST["pais_az"]) ? trim($_POST["pais_az"])  : 0;
    $canal = isset($_POST["canal_az"]) ? trim($_POST["canal_az"])  : 0;
    $sucursal = isset($_POST["suc_az"]) ? trim($_POST["suc_az"])  : 0;
    $folio = isset($_POST["folio_az"]) ? trim($_POST["folio_az"])  : 0;
    
    $numOnmy = isset($_POST["numOnmy"]) ? trim($_POST["numOnmy"])  : 0;
    $numView = isset($_POST["numView"]) ? trim($_POST["numView"])  : 0;
    if ($numOnmy > 0) {
      $numTel = substr($numOnmy, 2);
    } else {
      $numTel = $numView;
    }

    $idCamp = isset($_POST["id_camp_az"]) ? trim($_POST["id_camp_az"])  : 0;
    $idTarea = "0";

    $idEmp = isset($_POST["idNomAzteca"]) ? $_POST["idNomAzteca"]  : 0;
    $nomEmp = isset($_POST["nomAzteca"]) ? $_POST["nomAzteca"]  : '';
    $numEmp = $idEmp;
    ...
    $data = $json;
    //$data = '{"pais":"2","canal":"1","sucursal":"194","folio":"36700","numTel":"56897412","idCamp":"53","idTarea":"0","numEmp":"12354","token":"CobZa2018","idOrigen":"2","canalOrigen":"11","fecha":"02\/09\/2021","hora":"24:00:00","tipoCliente":"0"}';
    ...
    if ($on == 1) {
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://sclpcj.com.mx:7072/regional/cobranza/callcenter/cobranzaTelefonica.do?method=callCenter&jsonResp=' . $strEncript,
  ...
  ?>
    ...
    <div class="modal fade" id="modalAzteca" tabindex="-1" role="dialog" aria-labelledby="modalAzteca" aria-hidden="true">
      <div class="modal-dialog full-screen-azteca" role="document">
        <div class="modal-content full-screen-azteca">
          <div class="modal-body">
            <?php
            //print decrypt($strEncript,$key);
            ...
            ?>
            <a id="paginaAzteca" name="paginaAzteca" href="https://sclpcj.com.mx:7072/regional/cobranza/callcenter/cobranzaTelefonica.do?method=callCenter&jsonResp=<?php echo $strEncript; ?>" target="_blank"></a>
            <iframe width="100%" height="100%" src="https://sclpcj.com.mx:7072/regional/cobranza/callcenter/cobranzaTelefonica.do?method=callCenter&jsonResp=<?php echo $strEncript; ?>" frameborder="0" allowfullscreen></iframe>
...
while ($rTMP = ibase_fetch_assoc($v_query)) {
  ...
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["SEMANA"]              = $rTMP["SEMANA"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["QOTRENEG"]              = $rTMP["QOTRENEG"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["QOTCONVE"]              = $rTMP["QOTCONVE"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["NIVRIESGO"]              = $rTMP["NIVRIESGO"];

  $arrRegistroCliente[$rTMP["NUMTRANS"]]["PAIS"]              = $rTMP["PAIS"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["CANAL"]              = $rTMP["CANAL"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["SUCURSAL"]              = $rTMP["SUCURSAL"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["FOLIO"]              = $rTMP["FOLIO"];
  $arrRegistroCliente[$rTMP["NUMTRANS"]]["IDCAMPANA"]              = $rTMP["IDCAMPANA"];
}
...
if (is_array($arrRegistroCliente) && (count($arrRegistroCliente) > 0)) {
  ...
  foreach ($arrRegistroCliente as $rTMP['key'] => $rTMP['value']) {
    ...
    $SEMANA = $rTMP["value"]['SEMANA'];

    $QOTRENEG = $rTMP["value"]['QOTRENEG'];
    $QOTRENEG = number_format($QOTRENEG, 2);

    $NIVRIESGO = $rTMP["value"]['NIVRIESGO'];

//////////////////////////////////AZTECA

    $PAIS = $rTMP["value"]['PAIS'];
    $CANAL = $rTMP["value"]['CANAL'];
    $SUCURSAL = $rTMP["value"]['SUCURSAL'];
    $FOLIO = $rTMP["value"]['FOLIO'];
    $IDCAMPANA = $rTMP["value"]['IDCAMPANA'];
