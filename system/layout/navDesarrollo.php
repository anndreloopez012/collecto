<body class="hold-transition sidebar-mini layout-fixed sidebar-collapse ">
  <nav class=" navbar navbar-expand navbar-white navbar-light">
    <ul class="navbar-nav">
      <li class="nav-item d-none d-sm-inline-block">
        <a href="home.php" class="nav-link"><b><?php echo $title ?></b></a>
      </li>
    </ul>
    <ul class="navbar-nav ml-auto">
      <li class="nav-item" data-toggle="tooltip" data-placement="top" title="New Window">
        <div id="window">
          <a class="nav-link" href="<?php echo $return ?>" <i class="fas fa-arrow-from-right"></i>
          </a>
        </div>
      </li>
      <li class="nav-item" data-toggle="tooltip" data-placement="top" title="New Window">
        <div id="window">
          <a class="nav-link" href="<?php echo $windowHome ?>" <i class="fad fa-home-lg-alt"></i>
          </a>
        </div>
      </li>
      <li class="nav-item" data-toggle="tooltip" data-placement="top" title="New Window">
        <div id="window">
          <a class="nav-link" href="../../home.php" target="_blank">
            <i class="far fa-window-restore"></i>
          </a>
        </div>
      </li>
      <li class="nav-item" data-toggle="tooltip" data-placement="top" title="Log out">
        <div id="logout">
          <a class="nav-link" href="<?php echo $logout ?>">
            <i class="fas fa-times-circle"></i>
          </a>
        </div>
      </li>
    </ul>
  </nav>




  <style>
    /** onclick="myTimeEndGenIH()"           onclick="myTimeEndGenI()"              */

    div.scrmenu {
      text-align: right !important;
    }
  </style>